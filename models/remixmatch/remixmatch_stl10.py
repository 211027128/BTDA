...
class ReMixMatch:
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (_, x_ulb_w, x_ulb_s1, x_ulb_s2, x_ulb_s1_rot, rot_v) in zip(self.loader_dict['train_lb'],
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                if self.it>=0000:
                    est_q, cond_val = self.estimate_q_y(self.loader_dict['eval_estim'], self.loader_dict['eval_ulb'],self.model, 10)
                    is_zero = (est_q < 0).float().sum()
                    # no negative element and stable inverse
                    if is_zero == 0 and cond_val < 100:  # 100
                        n_infer += 1
                        final_q += est_q
                    #if self.it<50000 and is_zero > 0 and cond_val < 50:
                    #    n_infer += 1 
                    #    final_q += abs(est_q)
                    if(n_infer>0):
                        p_target = (final_q / n_infer).cuda(args.gpu)
                        p_target = p_target/torch.sum(p_target)

                    self.print_fn(f"####remix== n_infer{n_infer},cond_val:{cond_val}est_q:{est_q},q_model: {p_target}")
                ...
                self.print_fn(
                    f"{self.it} iteration, USE_EMA: {self.ema_m != 0}, {tb_dict}, BEST_EVAL_ACC: {best_eval_acc}, RECALL:{recall_best},at {best_it} iters")
                total_time = 0
    ...
    @torch.no_grad()
    def confusion(self,model, loader, num_class):
        ...
        for batch_idx, (_, inputs, targets) in enumerate(loader):
            ...
            for i in range(batch_size):
                confusion[:, int(targets[i])] += probs[i].cpu()
                num_classes[int(targets[i])] += 1

        return confusion

    @torch.no_grad()
    def estimate_q_y(self,val_loader, u_loader, model, num_class):
        model.eval()
        self.ema.apply_shadow()
        conf_val = self.confusion(self.model, val_loader, num_class)
        conf_unl = self.confusion(model, u_loader, num_class)
        ...
        q_y_tilde = q_y_tilde / torch.sum(q_y_tilde)
        q_y_esti = torch.matmul(inv_conf_val, q_y_tilde)

        self.print_fn(f"val:{conf_val},cond:{cond_val}")
        self.ema.restore()
        model.train()
