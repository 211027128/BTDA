...
class ReMixMatch:
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (_, x_ulb_w, x_ulb_s1, x_ulb_s2, x_ulb_s1_rot, rot_v,y_ulb) in zip(self.loader_dict['train_lb'],
                                                                                          self.loader_dict[
                                                                                              'train_ulb']):
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    '''
                    if self.it % 100==0:
                        be=F.kl_div(prob_x_ulb.log(),one_hot(y_ulb, args.num_classes, args.gpu))
                        before.append(be)
                    if self.it %5000==0:
                        self.print_fn(f"before:{before}")    
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                self.print_fn(
                    f"ReMixMatch=== {self.it} iteration,{tb_dict}, recal:{recall_best},BEST_EVAL_ACC: {best_eval_acc},at {best_it} iters")
                total_time = 0
