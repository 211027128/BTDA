...
from .abc_utils import consistency_loss, Get_Scalar, one_hot, mixup_one_target
        ...
        for (_, x_lb, y_lb), (_, x_ulb_w, x_ulb_s1, x_ulb_s2, x_ulb_s1_rot, rot_v) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                with torch.no_grad():
                    ...
                    y_lb=one_hot(y_lb, args.num_classes, args.gpu)
                    input_labels = torch.cat(
                        [y_lb, sharpen_prob_x_ulb, sharpen_prob_x_ulb,
                ...
                ir2=p_target[-1]/p_target
                ir22 = 1 - (self.it / args.num_train_iter) * (1 - ir2)
                if args.dataset != 'cifar100' and args.imbalancetype == 'step':
                    maskforbalance = torch.bernoulli(torch.sum(y_lb * torch.tensor(ir22).cuda(0), dim=1).detach())
                else:
                    maskforbalance = torch.bernoulli(torch.sum(y_lb * torch.tensor(ir2).cuda(0), dim=1).detach())

                pseudo_x=torch.softmax(logits_x_lb, dim=1)
                pseudo_x_w =torch.softmax(logits_x_ulb_w, dim=1)
                pseudo_x_s1 =torch.softmax(logits_x_ulb_s1, dim=1)
                pseudo_x_s2 =torch.softmax(logits_x_ulb_s2, dim=1)
                max_p,label_u=torch.max(pseudo_x_w, dim=1)
                select_mask = max_p.ge(0.95)
                label_u=one_hot(label_u, args.num_classes, args.gpu)
                maskforbalanceu = torch.bernoulli(torch.sum(label_u.cuda(0) * torch.tensor(ir22).cuda(0), dim=1).detach())

                abcloss = -torch.mean(maskforbalance * torch.sum(torch.log(pseudo_x) * y_lb.cuda(0), dim=1))
                abcloss1 = -torch.mean(select_mask * maskforbalanceu * torch.sum(torch.log(pseudo_x_s1) * pseudo_x_w.cuda(0).detach(), dim=1))
                abcloss2 = -torch.mean(select_mask * maskforbalanceu * torch.sum(torch.log(pseudo_x_s2) * pseudo_x_w.cuda(0).detach(), dim=1))
                ...
                total_loss = sup_loss + args.w_rot * rot_loss + w_kl * u1_loss + w_match * unsup_loss+abcloss
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                self.print_fn(
                    f"Abc=== {self.it} iteration,{tb_dict}, recal:{recall_best},BEST_EVAL_ACC: {best_eval_acc},at {best_it} iters")
                total_time = 0
