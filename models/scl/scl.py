...
from .scl_utils import consistency_loss, Get_Scalar, one_hot, mixup_one_target
    ...
    def train(self, args, logger=None):
        ...
        for (_, x_lb, y_lb), (_, x_ulb_w, x_ulb_s1, x_ulb_s2, x_ulb_s1_rot, rot_v) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                ...
                _, Nc = torch.max(logits_u, dim=1)
                # print(Nc)
                n_nmax=p_target/torch.max(p_target)
                print(n_nmax)
                g_nc = 0.5 ** (1 - n_nmax[Nc])
                ...
                # unsup_loss
                unsup_loss = ce_loss(logits_u, mixed_y[num_lb:], use_hard_labels=False) * g_nc
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                self.print_fn(
                    f"Scl=== {self.it} iteration,{tb_dict}, recal:{recall_best},BEST_EVAL_ACC: {best_eval_acc},at {best_it} iters")
                total_time = 0
