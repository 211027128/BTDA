...
class Linear:
    ...
    def train(self, args, logger=None):
        ...
        p_target=p_target/torch.sum(p_target).to(torch.float32)
        p_target = p_target.cuda(args.gpu)
        '''
        #self.print_fn( f"p_target:{p_target}{q_target}")
        p_model = None
        q_model = None
        p_std = None
        q_std = None
            ...
            with amp_cm():
                ...
                with torch.no_grad():
                    ...
                    if q_model == None:
                        ...
                        #qq_model = torch.mean(prob_x_lb.detach(), dim=0)
                        q_std = 1e-5
                    else:
                        #prob_x_lb1 = prob_x_lb * p_target / qq_model
                        q_model = q_model * 0.9 + torch.mean(prob_x_lb.detach(), dim=0) * 0.1
                        #qq_model = qq_model * 0.9 + torch.mean(prob_x_lb.detach(), dim=0) * 0.1
                    ...
                    prob_x_ulb_tran =  prob_x_ulb / p_model * p_target
                    prob_x_lb_tran = prob_x_lb / q_model * q_target
                    ''' 
                ...
                with torch.no_grad():
                    loss_weight = torch.sum(torch.abs(self.inverse_matrix.weight - torch.eye(args.num_classes).cuda()), dim=1)
                    loss_weight = torch.softmax(loss_weight, dim=0)

                    # hyper-params for update
                    T = self.t_fn(self.it)
                    # prob_x_ulb = self.inverse_matrix(prob_x_ulb)
                    ...
                    # logits_x_lb_t = self.inverse_matrix(logits_x_lb, u2l=False)
                    # prob_x_lb = torch.softmax(logits_x_lb_t, dim=1)
                    # sharpen_prob_x_lb = prob_x_lb ** (1 / T)
                    # sharpen_prob_x_lb = (sharpen_prob_x_lb / sharpen_prob_x_lb.sum(dim=-1, keepdim=True)).detach()

                    y_lb_t = one_hot(y_lb, args.num_classes, args.gpu)
                    # mix up
                    mixed_inputs = torch.cat((x_lb, x_ulb_s1, x_ulb_s2, x_ulb_w))
                    input_labels = torch.cat(
                        [y_lb_t, sharpen_prob_x_ulb, sharpen_prob_x_ulb,
                ...
                # ul_loss = ce_loss(ul_logits, sharpen_prob_x_lb, use_hard_labels=False)
