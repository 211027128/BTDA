...
from models.bis.bis_remix import Bis
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # SET Bis: class Bis in models.bis
    ...
    target = torch.tensor(lb_dset.targets)
    ...
    # random_sampler = data.RandomSampler(lb_dset, num_samples=args.batch_size * args.num_train_iter)

    random_samples_weight = torch.tensor([1 for t in target])
    random_sampler = data.WeightedRandomSampler(random_samples_weight, args.batch_size * args.num_train_iter)

    mean_sampler = data.WeightedRandomSampler(mean_samples_weight, args.batch_size * args.num_train_iter)
    reverse_sampler = data.WeightedRandomSampler(reverse_samples_weight, args.batch_size * args.num_train_iter)


    U_SAMPLES_PER_CLASS = class_sample_count * args.uratio
    # desi_p_m = torch.Tensor(U_SAMPLES_PER_CLASS).cuda()
    desi_p_m = U_SAMPLES_PER_CLASS
    desi_p_m = desi_p_m / desi_p_m.max()
    desi_p_m = torch.flip(desi_p_m, [0])
    desi_p_r = desi_p_m.pow(2)
    desi_p_m = torch.pow(desi_p_m, args.pseudo_esp).cuda()
    desi_p_r = torch.pow(desi_p_r, args.pseudo_esp).cuda()
    desi_p_o = torch.ones(desi_p_m.shape).cuda()

    if args.sampler1 == 'random':
        sampler1 = random_sampler
        desi_p1 = desi_p_o
    elif args.sampler1 == 'mean':
        sampler1 = mean_sampler
        desi_p1 = desi_p_m
    elif args.sampler1 == 'reverse':
        sampler1 = reverse_sampler
        desi_p1 = desi_p_r

    if args.sampler2 == 'random':
        sampler2 = random_sampler
        desi_p2 = desi_p_o
    elif args.sampler2 == 'mean':
        sampler2 = mean_sampler
        desi_p2 = desi_p_m
    elif args.sampler2 == 'reverse':
        sampler2 = reverse_sampler
        desi_p2 = desi_p_r

    loader_dict = {}
    dset_dict = {'train_lb1': lb_dset, 'train_lb2': lb_dset, 'train_ulb': ulb_dset, 'eval': eval_dset}

    batch_sampler1 = data.BatchSampler(sampler=sampler1, batch_size=args.batch_size, drop_last=True)
    batch_sampler2 = data.BatchSampler(sampler=sampler2, batch_size=args.batch_size, drop_last=True)

    loader_dict['train_lb1'] = data.DataLoader(lb_dset, batch_sampler=batch_sampler1, num_workers=args.num_workers)
    loader_dict['train_lb2'] = data.DataLoader(lb_dset, batch_sampler=batch_sampler2, num_workers=args.num_workers)
    ...
    # START TRAINING of Bis
    trainer = model.train
    for epoch in range(args.epoch):
        trainer(args, desi_p1, desi_p2, logger=logger)
...
if __name__ == "__main__":
    ...
    '''
    parser.add_argument('--alpha1', type=float, default=0.75, help='param for Beta distribution of Mix Up')
    ...
    # config file
    parser.add_argument('--c', type=str, default='./config/fixmatch/fixmatch_cifar10_4000_0.yaml')
    ...
    parser.add_argument('--pseudo-esp', default=1 / 3, type=float,
                        help='Exponential hyper-parameter for unlabeled data')
