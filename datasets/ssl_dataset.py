...
from .data_utils import split_ssl_data, sample_labeled_data ,split_imb_data
...
def mul_imb_factor(args, data, target, num_labels, num_classes):
    ...
    for c in range(num_classes):
        ...
        idx = idx[:img_num_per_cls[c]]
        lb_data.extend(data[idx])
        lbs.extend(target[idx])
    data, target = np.array(lb_data), np.array(lbs)
    data, target,_ = sample_labeled_data(args, data, target, num_labels, num_classes, None, im_num=img_num_per_cls)
    return data, target
class SSL_Dataset:
    """
        ...
        if 'CIFAR' in self.name.upper():
            ...
            if self.train:
                if svhn_extra:  # train+extra
                    ...
                    # dset_extra = dset(self.data_dir, split='extra', download=False)
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        if self.alg == 'fullysupervised':
            ...
            lb_data=lb_data[:num_labels]
            lb_targets=lb_targets[:num_labels]
            print(len(lb_data))
        ...
        if self.name.upper() == 'STL10':
            ...
            lb_data, lb_targets= mul_imb_factor(self.args, lb_data, lb_targets, num_labels, self.num_classes)
            num=len(ulb_data)
            #ulb_data, ulb_targets =mul_imb_factor(self.args, ulb_data, np.zeros(num), num, self.num_classes)
            ulb_targets=None
        else:
            data, targets = self.get_data()
            ...
            '''   
            #for true distribution
            lb_data, lb_targets, ulb_data, ulb_targets ,p_target,q_target= split_imb_data(self.args, data, targets,
                                                                        num_labels, self.num_classes,
                                                                        index, include_lb_to_ulb)                                                         
            '''
        if self.alg == 'fully_lt':
            lb_targets=np.concatenate([ulb_targets, lb_targets], axis=0)
            lb_data=np.concatenate([ulb_data, lb_data], axis=0)
            print(len(lb_data))
            lb_dset = BasicDataset(self.alg, lb_data, lb_targets, self.num_classes,
                                   self.transform, False, None, onehot)
            print("dataset with long_tail")
            return lb_dset, None
        ...
        out = {"distribution": p_target,"q_target":q_target}
        '''
