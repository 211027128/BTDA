...
from models.darp.darp_remix import Darp
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # SET darp: class darp in models.darp
    ...
    N_SAMPLES_PER_CLASS = torch.tensor([(target == t).sum() for t in torch.unique(target, sorted=True)])
    #num = len(N_SAMPLES_PER_CLASS)
    #need = args.num_classes - num
    #sec = torch.ones(need)
    #N_SAMPLES_PER_CLASS = torch.cat([N_SAMPLES_PER_CLASS, sec], 1)
    #U_SAMPLES_PER_CLASS = N_SAMPLES_PER_CLASS * args.uratio
    ...
    emp_distb_u = torch.ones(args.num_classes) / args.num_classes
    pseudo_orig = torch.ones(len(ulb_dset.data), args.num_classes) / args.num_classes
    pseudo_refine = torch.ones(len(ulb_dset.data), args.num_classes) / args.num_classes
    #target_disb = N_SAMPLES_PER_CLASS * len(ulb_dset.data) / sum(N_SAMPLES_PER_CLASS)
    #'''
    ...
    with open(dist_file_name, 'r') as f:
        ...
        #target_disb = p_target* len(ulb_dset.data)
    #target_disb=torch.tensor([0.0613, 0.0829, 0.0493, 0.1311, 0.0768, 0.1457, 0.0500, 0.0800, 0.0799,0.2429]* len(ulb_dset.data))
    print(p_target)
    
    '''
    random.seed(0)
    noisy=np.random.rand(10)
    p_target=p_target+noisy*0.1
    p_target=p_target/torch.sum(p_target)
    logger.info( f"p_target:{p_target}")
    '''
    target_disb=p_target* len(ulb_dset.data)
    #print(target_disb)
    trainer = model.train
    for epoch in range(args.epoch):
        trainer(args,emp_distb_u,target_disb,pseudo_orig,pseudo_refine, logger=logger)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mix_alpha', type=float, default=0.75, help='param for Beta distribution of Mix Up')
    ...
    parser.add_argument('--hard_label', type=str2bool, default=True, help='Whether to use distribution matching')
